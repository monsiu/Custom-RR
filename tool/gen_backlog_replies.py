...
def covered(n, codename, note=''):
    ...
    body = [
        "Hey, thanks for the request! \U0001f64f Good news: this device is **already in the catalog**. It is listed as `" + dev + "`.",
        "",
        "It is covered by:",
    ...
    if note: body += ["", note]
    body += ["", "Closing as already covered. If you meant a different variant that reports another codename, reply here and we will take another look. \U0001f44d"]
...
drafts.append(covered(129, 'frankel',
    note="Note GrapheneOS installs on Pixels via their official web installer, so no custom recovery is needed."))
...
drafts.append(dict(n=174, action='comment+label already-covered+close (completed)', text='\n'.join([
    "Hey, thanks for the request! \U0001f64f Good news: the OnePlus 8T is **already in the catalog** under its codename `kebab`. It is listed as `" + m167['matches'][0]['device'] + "`.",
    "",
    "It is covered by:",
...
drafts.append(manual(208, """
    Hey, thanks for the detailed request! \U0001f64f An honest status check: the **Redmi Note 14 5G (`tanzanite`)** has no dedicated entry in the catalog yet. The closest match we list is the **POCO X7 / Redmi Note 14 Pro 5G (`malachite`)** under LineageOS. That is a *different* model, so please don't flash malachite builds on tanzanite.
...
drafts.append(manual(156, """
    Duplicate of #208 (same device, `tanzanite`), tracking it there. Short version: no dedicated build in the catalog yet; the malachite entry you may find is the Note 14 **Pro**, a different model. See #208 for the GSI/XDA route in the meantime. \U0001f44d
...
drafts.append(manual(196, """
    Hey, thanks for the request! \U0001f64f Careful one: the catalog lists `cancunf` (moto g54/g64), and that is **not** the Moto G14 (`cancun`), despite the similar codename, so don't flash cancunf builds.
...
drafts.append(manual(158, """
    ...
    Closing as answered: the ROMs you can flash are already in the app. \U0001f44d
