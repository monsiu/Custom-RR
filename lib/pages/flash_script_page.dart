...
class _FlashScriptPageState extends State<FlashScriptPage> {
  ...
  /// Applies a state change and persists the new selections.
  void _set(VoidCallback change) {
    setState(change);
    _persist();
  }

  /// Restores the last-used selections, unless the page was opened with
  /// specific values (e.g. via a deep link), in which case those win.
  Future<void> _loadSelections() async {
    if (widget.initialBrand != null ||
        widget.initialCodename != null ||
        widget.initialRomId != null ||
        widget.initialRecoveryId != null) {
      return;
    }
    final SharedPreferences prefs = await SharedPreferences.getInstance();
    if (!mounted) return;
    setState(() {
      _brand = prefs.getString('${_kPrefsPrefix}brand');
      _codename = prefs.getString('${_kPrefsPrefix}codename');
      _romId = prefs.getString('${_kPrefsPrefix}romId');
      _recoveryId = prefs.getString('${_kPrefsPrefix}recoveryId');
      _wantsGapps = prefs.getBool('${_kPrefsPrefix}gapps') ?? true;
      _wantsMagisk = prefs.getBool('${_kPrefsPrefix}magisk') ?? false;
      _wipeData = prefs.getBool('${_kPrefsPrefix}wipe') ?? true;
      _gsiMode = prefs.getBool('${_kPrefsPrefix}gsi') ?? false;
    });
  }

  /// Saves the current selections so they survive leaving the page.
  Future<void> _persist() async {
    ...
    await put('${_kPrefsPrefix}brand', _brand);
    await put('${_kPrefsPrefix}codename', _codename);
    await put('${_kPrefsPrefix}romId', _romId);
    await put('${_kPrefsPrefix}recoveryId', _recoveryId);
    await prefs.setBool('${_kPrefsPrefix}gapps', _wantsGapps);
    await prefs.setBool('${_kPrefsPrefix}magisk', _wantsMagisk);
    await prefs.setBool('${_kPrefsPrefix}wipe', _wipeData);
    await prefs.setBool('${_kPrefsPrefix}gsi', _gsiMode);
  }
  ...
  String _generate() {
    final CatalogRepository repo = CatalogRepository.instance;
    return buildFlashScript(
      brand: _brand ?? '',
      codename: _codename ?? '',
      rom: _romId == null ? null : repo.romById(_romId!),
      recovery: _recoveryId == null ? null : repo.recoveryById(_recoveryId!),
      wantsGapps: _wantsGapps,
          ...
          child: ListView(
            ...
            children: <Widget>[
              Text(
                ...
                'run each block yourself; it is not a one-tap installer. Use '
                'Copy or Save / share to get the script onto your computer.',
              ...
              SwitchListTile(
                ...
                value: _gsiMode,
                onChanged: (bool v) => _set(() => _gsiMode = v),
              ),
              const SizedBox(height: 12),
              if (!_gsiMode) ...<Widget>[
                _LabelledDropdown<String>(
                  ...
                  onChanged: (String? v) => _set(() => _romId = v),
                ),
                const SizedBox(height: 12),
              ],
              _LabelledDropdown<String>(
                ...
                onChanged: (String? v) => _set(() => _recoveryId = v),
              ),
              ...
              SwitchListTile(
                ...
                subtitle: Text(
                  _gsiMode
                      ? 'Prefer a GApps/GMS GSI build. Non-Samsung Treble has '
                          'no recovery to flash a separate gapps.zip.'
                      : 'Adds a separate gapps.zip flash step.',
                ),
                value: _wantsGapps,
                onChanged: (bool v) => _set(() => _wantsGapps = v),
              ),
              SwitchListTile(
                title: const Text('Include Magisk (root)'),
                subtitle: Text(
                  _gsiMode
                      ? 'Samsung: flash magisk.zip in recovery. Otherwise patch '
                          'boot.img with the Magisk app.'
                      : 'Flash Magisk last so it patches the new boot.',
                ),
                value: _wantsMagisk,
                onChanged: (bool v) => _set(() => _wantsMagisk = v),
              ),
              SwitchListTile(
                ...
                value: _wipeData,
                onChanged: (bool v) => _set(() => _wipeData = v),
              ),
              const SizedBox(height: 16),
              if (!ready)
...
/// SharedPreferences key prefix for the remembered generator selections.
const String _kPrefsPrefix = 'flashScript.sel.';
...
String buildFlashScript({
  ...
  CatalogEntry? rom,
  CatalogEntry? recovery,
  required bool wantsGapps,
  required bool wantsMagisk,
  ...
  b.writeln('# Options: gapps=$wantsGapps, magisk=$wantsMagisk, '
      'wipeData=$wipeData');
  b.writeln('#');
...
/// Pure builder for the GSI / Treble flash script. Top-level and fully
/// parameterised so it can be unit-tested. [brand] may be empty; there is no
/// codename because the GSI flow is identical across a brand's Treble devices.
@visibleForTesting
String buildGsiScript({
  required String? brand,
  CatalogEntry? recovery,
  required bool wantsGapps,
  required bool wantsMagisk,
  ...
  final CatalogEntry? rec = recovery;
  final bool samsung = (brand ?? '').toLowerCase() == 'samsung';
  final StringBuffer b = StringBuffer();
  b.writeln('#!/usr/bin/env bash');
  b.writeln('# Generated by Custom RR · flash-script helper (GSI / Treble)');
  b.writeln('# Author : Monsiu (https://github.com/monsiu)');
  b.writeln('# Source : https://github.com/monsiu/Custom-RR');
  b.writeln('# Target : ${(brand ?? '').isEmpty ? 'Any' : brand} '
